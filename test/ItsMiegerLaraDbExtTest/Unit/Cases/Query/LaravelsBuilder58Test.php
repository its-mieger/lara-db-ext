<?php
	...
	/**
	 * This test class is copied from Laravel's DatabaseQueryBuilderTest (v5.8) to ensure core functionality is not broken by this extension
	 * @package ItsMiegerLaraDbExtTest\Unit\Cases\Query
	 */
	class LaravelsBuilder58Test extends TestCase
	{
		...
		public function testBasicSelectWithGetColumns() {
			...
			$builder->getConnection()->shouldReceive('cursor')->once()->andReturnUsing(function ($sql) use ($builder) {
				$this->assertEquals('select * from "users"', $sql);

				$fn = $this->connCursorReturn($builder, []);
		...
		public function testBasicSelectUseWritePdo() {
			...
			$builder->getConnection()->shouldReceive('cursor')->once()
				...
				->andReturnUsing($this->connCursorReturn($builder, []));
		...
		public function testHavingFollowedBySelectGet() {
			...
			$builder->getConnection()->shouldReceive('cursor')->once()->with($query, ['popular', 3], true)->andReturn([['category' => 'rock', 'total' => 5]])->andReturnUsing($this->connCursorReturn($builder, [['category' => 'rock', 'total' => 5]]));
			...
			$builder->getConnection()->shouldReceive('cursor')->once()->with($query, ['popular'], true)->andReturn([['category' => 'rock', 'total' => 5]])->andReturnUsing($this->connCursorReturn($builder, [['category' => 'rock', 'total' => 5]]));
			$builder->getProcessor()->shouldReceive('processSelect')->andReturnUsing(function ($builder, $results) {
				return $results;
			});
			$builder->from('item');
		...
		public function testGetCountForPaginationWithBindings() {
			...
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select count(*) as aggregate from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 1]]));
		...
		public function testGetCountForPaginationWithColumnAliases() {
			...
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select count("body", "teaser", "posts"."created") as aggregate from "posts"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 1]]));
			$builder->getProcessor()->shouldReceive('processSelect')->once()->andReturnUsing(function ($builder, $results) {
				return $results;
			});
		...
		public function testGetCountForPaginationWithUnion() {
			...
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select count(*) as aggregate from (select "id" from "posts" union select "id" from "videos") as "temp_table"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 1]]));
			$builder->getProcessor()->shouldReceive('processSelect')->once()->andReturnUsing(function ($builder, $results) {
				return $results;
			});
		...
		public function testFindReturnsFirstResultByID() {
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select * from "users" where "id" = ? limit 1', [1], true)->andReturnUsing($this->connCursorReturn($builder, [['foo' => 'bar']]));
		...
		public function testPluckMethodGetsCollectionOfColumnValues() {
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->andReturnUsing($this->connCursorReturn($builder, [['foo' => 'bar'], ['foo' => 'baz']]));
			...
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->andReturnUsing($this->connCursorReturn($builder, [['id' => 1, 'foo' => 'bar'], ['id' => 10, 'foo' => 'baz']]));
		...
		public function testValueMethodReturnsSingleColumn() {
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select "foo" from "users" where "id" = ? limit 1', [1], true)->andReturnUsing($this->connCursorReturn($builder, [['foo' => 'bar']]));
		...
		public function testAggregateFunctions() {
			...
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select max("id") as aggregate from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 1]]));
			$builder->getProcessor()->shouldReceive('processSelect')->once()->andReturnUsing(function ($builder, $results) {
				return $results;
			});
			...
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select min("id") as aggregate from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 1]]));
			$builder->getProcessor()->shouldReceive('processSelect')->once()->andReturnUsing(function ($builder, $results) {
				return $results;
			});
			...
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select sum("id") as aggregate from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 1]]));
			$builder->getProcessor()->shouldReceive('processSelect')->once()->andReturnUsing(function ($builder, $results) {
				return $results;
			});
		...
		public function testAggregateResetFollowedByGet() {
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select count(*) as aggregate from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 1]]));
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select sum("id") as aggregate from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 2]]));
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select "column1", "column2" from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['column1' => 'foo', 'column2' => 'bar']]));
			$builder->getProcessor()->shouldReceive('processSelect')->andReturnUsing(function ($builder, $results) {
				return $results;
			});
		...
		public function testAggregateResetFollowedBySelectGet() {
			$builder = $this->getBuilder();
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select count("column1") as aggregate from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['aggregate' => 1]]));
			$builder->getConnection()->shouldReceive('cursor')->once()->with('select "column2", "column3" from "users"', [], true)->andReturnUsing($this->connCursorReturn($builder, [['column2' => 'foo', 'column3' => 'bar']]));
			$builder->getProcessor()->shouldReceive('processSelect')->andReturnUsing(function ($builder, $results) {
				return $results;
			});
			$builder->from('users');
...
SQL;
		...
		protected function connCursorReturn(Builder $builder, $data) {

			return function () use ($data, $builder) {


				$pdoMock = m::mock(\PDOStatement::class);
				$pdoMock->shouldReceive('setFetchMode')->with(\PDO::FETCH_NUM)->once();

				if (empty($data)) {

					$pdoMock->shouldReceive('columnCount')->never();
					$pdoMock->shouldReceive('getColumnMeta')->never();
				}
				else {
					$columns = array_keys($data[0]);

					$pdoMock->shouldReceive('columnCount')->once()->andReturn(count($columns));
					$pdoMock->shouldReceive('getColumnMeta')->andReturnUsing(function ($index) use ($columns) {
				...
				$dispatcher->dispatch(new StatementPrepared($builder->getConnection(), $pdoMock));
